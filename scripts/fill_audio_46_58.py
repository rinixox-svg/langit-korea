...
# -*- coding: utf-8 -*-
"""
Extract audio script (듣기지문) dari PDF unit 46 dan 58
lalu isi field 'audio_teks' di listening_data.json
"""
...
print("=== EKSTRAKSI AUDIO SCRIPT UNIT 46 & 58 ===\n")
...
if not (pdf_dir / unit_pdf_map[58]).exists():
    # Cari file yang mirip
    for f in pdf_dir.iterdir():
        if "58" in f.name and "labor" in f.name.lower():
            unit_pdf_map[58] = f.name
            break


def extract_audio_script(pdf_path, unit_num):
    ...
    # Cari bagian "듣기지문" di PDF
    # Biasanya di halaman terakhir (halaman 10)
    audio_script = ""
    ...
    for page_num in range(len(doc)):
        ...
        if "듣기지문" in text:
            ...
            if len(parts) > 1:
                ...
                # Bersihkan: hapus nomor halaman di akhir
    ...
    doc.close()
    return audio_script if audio_script else "Tidak ditemukan bagian 듣기지문"
...
for unit_num, pdf_name in unit_pdf_map.items():
    ...
    audio_script = extract_audio_script(pdf_path, unit_num)

    if (
        not audio_script
        or audio_script.startswith("Error")
        or audio_script.startswith("Tidak")
    ):
...
laporan_path = output_dir / "laporan_audio_script_46_58.json"
...
print("\n=== SELESAI ===")
print("Audio script sudah terisi untuk unit 46 dan 58!")
print("Sekarang semua 150 soal listening sudah memiliki audio_teks!")
