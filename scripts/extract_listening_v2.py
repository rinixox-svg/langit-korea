...
def check_gambar_pilihan(unit_dir, soal_list, unit_num):
    """
    ...
    for soal in soal_list:
        nomor = soal["nomor"]
        # Cek apakah ada file gambar untuk soal ini
        for ext in [".jpeg", ".jpg", ".png"]:
            img_pattern = f"u{unit_num}_h9_soal_listening_{nomor:02d}{ext}"
            img_path = images_dir / img_pattern
            if img_path.exists():
...
def main():
    ...
    for unit_num in range(31, 61):
        ...
        # Cek gambar pilihan
        if unit_dir.exists():
            listening_soal = check_gambar_pilihan(unit_dir, listening_soal, unit_num)
